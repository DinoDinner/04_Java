...
public class ObjectArray {
	...
	public static void main(String[] args) {
		...
		MemberVO[] mArray2 = new MemberVO[10]; // 표준적으로 배열을 초기화 하는 문장
...
//		mArray2[0].setName("홍길동"); // 죽는 이유 : 배열은 생성하였지만 Member객체가 null이어서 죽음
...
//		mArray2[0].getName().length(); // 죽는 이유 : 객체 생성하고 이름을 초기화하지 않고 길이를 불러서 죽음.
		...
		// 객체 배열을 올바르게 초기화 하는 방법
		// 1. 객체 배열을 선언하고 객체 배열을 알맞은 크기로 초기화 한다.
		// 2. 실제 사용할 객체를 new로 생성해서 배열에 넣는다.
		// 3. 배열에 객체를 생성하지 않은 null로 되어 있는 영역은 접근하지 않는다! -> null check 필수!
		MemberVO[] mArray4 = new MemberVO[10]; 
		mArray4[0] = new MemberVO("test1", "홍길동", 24);
		mArray4[1] = new MemberVO("test2", "박길동", 25);
		mArray4[2] = new MemberVO("test3", "최길동", 26);
		if(mArray4[0] != null) { // null 체크하는 방법!
			System.out.println(mArray4[0].getName());
		}
		if(mArray4[3] != null) { // null 체크하는 방법!
		...
		// 만일 데이터가 중간에 삭제된다면? - null check가 들어가야 한다.
