...
// Person : 한 사람의 개인정보를 추상화 시켜 놓은 객체, 추상화 : 일부만 활용한다!
public class Person { // this가 찍는 시작점!!
	// 이곳을 필드라 부른다.
	// 선언부 - 객체 상단에서 멤버 변수를 선언하는 곳
	// 멤버 변수란? 객체가 가질수 있는 데이터
	...
	// 파라메터(인자)란? : 메소드(함수) 외부에서 입력 값으로 활용되는 변수, 함수의 로컬 변수로 들어간다.
	// this : 자기 자신을 가르치는 키워드, 필드로 돌아와서 멤버변수나 메소드를 찍을수 있는 키워드
...
//		name = "홍길동";
		this.name = name; // 멤버변수의 name = 지역변수 파라메터의 name
	}
	
	// void method(인자1, 인자2) : 리턴 값은 없고 인자가 2개있는 메소드
	public void setNameAndAddress(String name, String address) {
		this.name = name; // 멤버변수의 name = 지역변수 파라메터의 name
		this.address = address; // 멤버변수의 address = 지역변수 파라메터의 address간의 대입연산
	}
	
	// String method() : 인자가 없고 리턴값만 있는 형태
 	// getter의 표준 형태 - read only(읽기 전용)
	public String getName() {
		return name; // this.name과 동치, 인자나 로컬변수에 name 변수가 없음으로 멤버변수 name을 찾아 온다.
	}
	...
	// String method(인자) : 인자가 1개있고, 리턴값이 존재하는 메소드
