...
public class String2 {
	public static void main(String[] args) {
		...
		// 3. "".compareTo : 문자열 비교할때 쓴다. 대소비교까지 된다. 향후 문자열 정렬할때 사용됨
		System.out.println("홍길동".compareTo("김길동")); // 문자열 사전순 대소비교 가능
		// 같으면 0;
		// 작으면 -1
		// 크면 +1;
...
//              012345678		       
		...
		String token = "안녕";
		int startIndex = 0; // 문자열을 마지막으로 찾은 위치
		int count = 0;  // 갯수
		while(true) {
		...
		System.out.println(str5.lastIndexOf("안녕", 46 - "안녕".length())); // 찾은 위치 - 2, 46
		System.out.println("----------------------------------------------");
		...
		// 9. endsWith : 문자열의 끝 패턴을 확인, 주로 확장자 확인할때 쓰는 용도 ★★★★★
		...
		// 10. trim 류 : white space를 제거하는 함수 - strip추천!!
		...
		System.out.println(origin.strip()); // strip : 모든 white space 제거, 매우 빠름 ★★★★★
		...
		String text = origin.strip(); // strip 추천. 가장빠르고 많이 제거해준다.
...
//		String id = "test";
		String id = "'admin' and PW = '1' or '1' = '1'";
		// --> ID, PW 입력 받을때 특수문자 제거 처리 -> 보안상 필수 처리!
		...
		// 14. replaceFirst : 처음문자만 교체할 때
		str = "AAABBCC";
		System.out.println(str.replaceFirst("A", "Z"));
		System.out.println("----------------------------");
		
		
		text = text.replace("#*", ""); // 필요 없는 문자열 제거!
		System.out.println(text);
		// 15. split : 문자열을 특정 인자(Delimiter)구분자로 잘라서 토큰(Token)으로 반환하는 기능
