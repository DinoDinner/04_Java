...
public class LoopStatement1 {
	...
	public static void myFirstWhileLoop() {
		int count = 0;
		while (true) { // true인 경우를 무한 반복문 혹은 무한 루프문라고 한다.
...
//	[0][1][2]
	public static void testWhileLoop() {
		int i = 0; // 초기화문, i = index의 약자, 0부터 시작하는것이 정석적
	...
	public static void testForLoop() {
		...
		// 0부터 시작하고 9까지 돌고, 10은 도달하지 않는 for문
		for(int i = 0; i < 10; i++) { // ++i, i++ 중요하지 않음!
			System.out.println(i);
		}
...
//		int i = 0; // 라인을 줄이기 위해 for문 안으로 들어감!
		...
		// 패턴2 -> 항상 기본형을 타이핑을하고 i를 조작하여 출력하는 것을 권장함.
		for(int i = 0; i < 10; i++) { // 0 ~ 9 -> 1 ~ 10?
			System.out.println(i + 1); // i++ 절대 아님!!!!
		}
